...
class generic_kv:
    ...
    @classmethod
    def _convert_to_binary_query(cls, key = "", value = None, expire_sec: int = 0, bulk: dict = None):
        return json.dumps({
            "request": cls.request,
    ...
    def insert_key(cls, key: str, expire_sec: int = 0, persistent: bool = False):
        ...
        query = cls._convert_to_binary_query(key, expire_sec=expire_sec)
        return cls._run_query(query)
    
    @classmethod
    def insert_key_value_pair(cls, key: str, value, expire_sec: int = 0, persistent: bool = False):
        cls.command = "insert_key_value"
        cls.persistent = persistent
        query = cls._convert_to_binary_query(key, value, expire_sec=expire_sec)
        return cls._run_query(query)
    
    @classmethod
    def get_value(cls, key: str):
        cls.command = "get_value"
        query = cls._convert_to_binary_query(key)
        return cls._run_query(query)
        
    @classmethod
    def get_keys(cls):
        cls.command = "get_keys"
        query = cls._convert_to_binary_query()
        return cls._run_query(query)
    ...
    def update_value(cls, key: str, value):
        ...
        query = cls._convert_to_binary_query(key, value)
        return cls._run_query(query)
    ...
    def insert_bulk(cls, bulk: dict, expire_sec: int = 0, persistent: bool = False):    
        ...
        query = cls._convert_to_binary_query(bulk=bulk, expire_sec=expire_sec)
        return cls._run_query(query)
    
    @classmethod
    def delete_bulk(cls, bulk: list):
        cls.command = "delete_bulk"
        query = cls._convert_to_binary_query(bulk=bulk)
        return cls._run_query(query)
